...
class ICAOSeparationController:
    ...
    def _issue_resolution(self, ac1_id, ac2_id, alt1, alt2):
        # Simple resolution: climb one, descend the other
        ...
        if not self._already_assigned_and_reached(ac1_id, new_alt1, alt1):
            cmd1 = f"{ac1_id} ALT {new_alt1}"
            print(f"[RESOLVE] {cmd1}")
            stack(cmd1)
            self.assigned_altitudes[ac1_id] = new_alt1

        if not self._already_assigned_and_reached(ac2_id, new_alt2, alt2):
